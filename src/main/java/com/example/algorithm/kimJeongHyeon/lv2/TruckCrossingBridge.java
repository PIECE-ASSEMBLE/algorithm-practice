 ...
 * 
 * solution 함수의 매개변수로 다리 길이 bridge_length, 다리가 견딜 수 있는 무게 weight, 트럭별 무게
 * truck_weights가 주어집니다. 이때 모든 트럭이 다리를 건너려면 최소 몇 초가 걸리는지 return 하도록 solution 함수를
 * 완성하세요.
 * 
...
public class TruckCrossingBridge {
	public int solution(int bridge_length, int weight, int[] truck_weights) {
		...
		while (truckSeq < truck_weights.length) {
			answer++;

			if (!bridge.isEmpty() && (answer - bridge.peek().comeTime) >= bridge_length)
				truckWeight -= bridge.poll().weight;
			
			if (weight >= truckWeight + truck_weights[truckSeq]) {
				truckWeight += truck_weights[truckSeq];
				bridge.add(new Truck(truck_weights[truckSeq], answer));
				truckSeq++;
			}
		}
