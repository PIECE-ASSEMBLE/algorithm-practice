 ...
 * 
 * <p>필기</p>
 * '차곡차곡' 쌓여있다는 말을 잘 생각해야한다.
 * 나는 각 배열들이 한 x축을 의미하는 줄 알았으나, 말 그대로 첫번째 배열부터 위부터 붙기 시작해서 마지막이 가장 아래로 가는 각 배열이 y축을 이루는 배열임을 뒤늦게 알았다.
 * 이 문제에서 의미하는 2차원 배열은 정위치로 생각하면 되는 배열 (왼쪽 상단 -> 우측 하단)으로 흐르는 배열이다.
 * 
 * 
 * @author kaeao
 *
 */
public class CrainPickDollGame {
	...
	public int solution(int[][] board, int[] moves) {
		...
		for(int i=0; i<moves.length; i++) {
			int move = moves[i]-1;
			for(int j=0; j<=board[move].length-1; j++) {
